...
def CosineProcessing(Output_path_Prefix, Data_path_Prefix):
    CWD = os.getcwd()
    ...
    def FindMaxSimi(tdata):
        tfidf_testing = tfidf_vectorizer.transform(tdata)
        output = cosine_similarity(tfidf_matrix, tfidf_testing)
    ...
    file_cosine_similarity_output = open(
        CWD+"/"+Output_path_Prefix+"/CosineSimilarityOutput.txt", "w+", encoding='utf-8')
    #file_simi_score = open(CWD+"/Output/CosSimi.txt", "w+", encoding='utf-8')
    file_line_mapping_simi = open(
        CWD+"/"+Output_path_Prefix+"/CosSimi.txt", "w+", encoding='utf-8')
    ...
    for Tdoc in InputQueryData:
        ...
        newDindex, newmaxsim = FindMaxSimi(tlist)
        PunjSent = OriginalInputData[LNo-1]
        SD = AllCorpus[newDindex-1]
        Sent = str(LNo)+": "+SD+"\n"+PunjSent+"\n\n"
        file_cosine_similarity_output.write(
            str(newDindex)+": "+SD+"\n"+str(LNo)+": "+PunjSent+"\n\n")
    ...
    file_cosine_similarity_output.close()
    file_line_mapping_simi.close()
    print("\n")
    print("Source Sen:", len(AllCorpus))
    print("Target Sen:", len(InputQueryData), "\n")
    #print("Accuracy:", round(Acc/totDoc,3))
    ThresholdVal = sum(AllSimiScores)/len(AllSimiScores)
    
    f1 = open(CWD+"/"+Output_path_Prefix+"/CosSimiTh.txt", "w+", encoding='utf-8')
    f = open(CWD+"/"+Output_path_Prefix+"/CosSimi.txt", "r", encoding='utf-8')
    data=f.readlines()
    for i in data:
    ...
    file_cosine_similarity_output = open(CWD+"/"+Output_path_Prefix+"/CosineSimilarityOutputThreshold.txt",
                                         "w+", encoding='utf-8')
    ...
    for Sent in AllOUTPUTData:
        ...
        if float(score) >= ThresholdVal:
            file_cosine_similarity_output.write(Sent.split('@')[1])
            THcount += 1
